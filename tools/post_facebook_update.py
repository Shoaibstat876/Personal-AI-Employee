...
def build_post(source_name: str, source_text: str, title: str | None) -> str:
    ...
    preview = cleaned[:900]
    heading = title.strip() if title else f"Facebook Update — {source_name}"
    return f"""# {heading}

Project update from Personal AI Employee

Highlights:
- Continued workflow improvement
- Evidence-first execution
- Stronger logging and traceability
...
Status:
This update is ready for manual Facebook posting.
"""
...
def main() -> int:
    ...
    artifact_name = f"FACEBOOK_POST_{utc_stamp()}.md"
    ...
    with (LOGS / "facebook_execution_log.jsonl").open("a", encoding="utf-8") as f:
        f.write(json.dumps({
            "timestamp": utc_iso(),
            "workflow": "facebook",
    ...
    print(f"[OK] Facebook artifact created: {artifact_path}")
    return 0
