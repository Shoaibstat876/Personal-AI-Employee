...
def build_post(source_name: str, source_text: str, title: str | None) -> str:
    ...
    preview = cleaned[:220]
    heading = title.strip() if title else "Personal AI Employee update"
    return f"{heading}: {preview} #AI #Automation #Python"
...
def main() -> int:
    ...
    artifact_name = f"TWITTER_POST_{utc_stamp()}.md"
    ...
    with (LOGS / "twitter_execution_log.jsonl").open("a", encoding="utf-8") as f:
        f.write(json.dumps({
            "timestamp": utc_iso(),
            "workflow": "twitter",
    ...
    print(f"[OK] Twitter artifact created: {artifact_path}")
    return 0
