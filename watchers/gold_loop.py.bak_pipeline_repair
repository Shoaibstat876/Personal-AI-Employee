...
def _demo_transient_once_marker(task: str) -> _Path:
    """
    Judge-safe demo only:
    - If task starts with DEMO_RETRY_, simulate a transient error ONCE.
    - Marker stored under AI_Employee_Vault/Logs (ignored by git).
    """
    repo_root = _Path(__file__).resolve().parents[1]
    logs_dir = repo_root / "AI_Employee_Vault" / "Logs"
    logs_dir.mkdir(parents=True, exist_ok=True)
    safe = "".join([c if c.isalnum() or c in ("-", "_", ".") else "_" for c in str(task)])
    return logs_dir / f".demo_transient_once_{safe}.marker"
...
def run_once() -> int:
    ...
    if tasks:
        ...
        # Phase 5: bounded retry/backoff for transient failures (judge-safe)
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            append_log("RETRY_ATTEMPT", {"task": task0, "attempt": attempt, "max": max_attempts})

            try:

                                # Phase 5 demo: force ONE transient error for DEMO_RETRY_* tasks
                if str(task0).startswith('DEMO_RETRY_'):
                    mm = _demo_transient_once_marker(str(task0))
                    if not mm.exists():
                        mm.write_text('created', encoding='utf-8')
                        raise OSError('SIMULATED_TRANSIENT_LOCK_ERROR_ONCE')
                res = try_acquire_lock(task0, lock_id=RUN_ID)

                append_log("RETRY_SUCCESS", {"task": task0, "attempt": attempt})
                ...
                append_log("RETRY_TRANSIENT_ERROR", {"task": task0, "attempt": attempt, "error": str(e)})

                if attempt >= max_attempts:

                    append_log("RETRY_GIVEUP", {"task": task0, "attempt": attempt, "final": "giveup"})

                    st = load_state()

                    st["run_id"] = RUN_ID

                    st["inflight_task_id"] = task0

                    st["inflight_step"] = "RETRY_GIVEUP"

                    st["last_status"] = "failed_transient_giveup"

                    st["last_checkpoint"] = "RETRY_GIVEUP"

                    st["last_output_paths"] = []
                ...
                append_log("RETRY_BACKOFF_SLEEP", {"task": task0, "seconds": wait_s, "attempt": attempt})
        ...
        if res is None:
            ...
            raise SystemExit("RETRY_UNREACHABLE: no result object")
